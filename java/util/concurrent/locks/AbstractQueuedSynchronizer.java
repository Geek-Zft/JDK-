...
public abstract class AbstractQueuedSynchronizer
     ...
     *
     * <p>The wait queue is a variant of(一个变种) a "CLH" (Craig, Landin, and
     ...
     * releases.  Each node of the queue otherwise serves as(作为) a
     * specific-notification-style(具体通知式) monitor holding a single waiting
     * thread. The status field does NOT control whether threads are
     * granted locks etc though.  A thread may try to acquire if it is
     * first in the queue. But being first does not guarantee success;
     * it only gives the right to contend(竞争).  So the currently released
     ...
     * predecessor(前任). For explanation of similar mechanics in the case
     * of spin locks, see the papers by Scott and Scherer at
     * http://www.cs.rochester.edu/u/scott/synchronization/
     *
     * <p>We also use "next" links to implement blocking mechanics(阻塞机制).
     ...
     *
     * <p>CLH queues need a dummy(虚假) header node to get started. But
    ...
    static final class Node {
        /** Marker to indicate a node is waiting in shared mode 共享节点模式 */
        static final Node SHARED = new Node();
        /** Marker to indicate a node is waiting in exclusive mode 独占节点模式*/
        static final Node EXCLUSIVE = null;

        /** waitStatus value to indicate thread has cancelled
         *
         * 节点操作因为超时或者对应的线程被interrupt。节点不应该留在此状态，一旦达到此状态将从CLH队列中踢出
         * */
         ...
         *               since intervened.
         *   0:          None of the above  正常状态，新生的非CONDITION节点都是此状态
         *
         * The values are arranged numerically to simplify use.
         * Non-negative(非负) values mean that a node doesn't need to
         ...
         * cancellation of a predecessor, we short-circuit(短路) while
         ...
         * construction and nulled out after use. 节点绑定的线程
         */
     ...
     * This operation has memory semantics(语义) of a <tt>volatile</tt> read.
    ...
    private Node enq(final Node node) {
        ...
        for (;;) {
            Node t = tail;
            if (t == null) { // Must initialize  队列为空则创建头结点
    ...
    /**
     * Attempts to acquire in exclusive mode(独占模式). This method should query
     * if the state of the object permits it to be acquired in the
     * exclusive mode, and if so to acquire it.
     *
     * <p>This method is always invoked by the thread performing(执行)
