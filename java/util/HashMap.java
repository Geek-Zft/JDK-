 ...
 * An index of table is logically known as bucket(桶) and it stores first element of linkedlist
...
public class HashMap<K,V>
    extends AbstractMap<K,V> //区别于HashTable
    implements Map<K,V>, Cloneable, Serializable
{
     ...
     * {@code -1} value ensures that alternative（替代） hashing is never used.
     */
    ...
    final int hash(Object k) {
        ...
        //一次hash,调用k的hashCode方法，然后与hashSeed做异或运算
        h ^= k.hashCode();
     ...
     * others.
     */
    //如果key为null，则其hash值为0，默认是放在table[0]里的
    ...
    final Entry<K,V> getEntry(Object key) {
        ...
        //calculate the exact index then iterate through the linkedlist
        int hash = (key == null) ? 0 : hash(key);
        for (Entry<K,V> e = table[indexFor(hash, table.length)];
             e != null;
             e = e.next) {
            Object k;
            //check for key equality , if true return the Entry
            if (e.hash == hash &&
    ...
    public V put(K key, V value) {
        ...
        // HashMap允许key为null, 使用第0个桶存放
        if (key == null)
            return putForNullKey(value);
        //This hashcode is used to find index of array for storing Entry object
        //It may happen sometimes that, this hashcode function is poorly written
        //so JDK designer has put another function called hash() which takes above calculated hash value as argument
        int hash = hash(key);
        //used to calculate exact（精确的） index in table array for storing the Entry object.
        int i = indexFor(hash, table.length);

        /**
         * if two key objects have same hashcode(which is known as collision) then it will be stored in form of linkedlist.So here, we will iterate through our linkedlist
         */
        for (Entry<K,V> e = table[i]; e != null; e = e.next) {
            Object k;
            //What if we are putting same key again, logically it should replace old value.
            // Yes,it will do that.While iterating it will check key equality by calling equals() method(key.equals(k)),
            // if this method returns true then it replaces value object with current Entry’s value object.
    ...
    static class Entry<K,V> implements Map.Entry<K,V> {
        //The answer  is, hash code is calculated for a key by calling Hascode() method. This hashcode is used to calculate index for above Entry[] table.
        final K key;
        V value;
        Entry<K,V> next;
        int hash;
